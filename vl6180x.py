...
_VL6180X_REG_HIST_RESULT = const(0x052)

_VL6180X_REG_SYSTEM__HISTORY_CTRL = const(0x0012)

_VL6180X_REG_SYSTEM__INTERRUPT_CONFIG_GPIO = const(0x014)

_VL6180X_REG_SYSRANGE__INTERMEASUREMENT_PERIOD = const(0x01B)
...
#  writeReg System__Fresh_OUT_OF_Reset
    ...
    def startContRange(self):
    	self.writeByte(_VL6180X_REG_SYSRANGE__INTERMEASUREMENT_PERIOD, 0x0A) 
    	self.writeByte(_VL6180X_REG_SYSTEM__INTERRUPT_CONFIG_GPIO, 0x20)  # Configures interrupt disabled
    	self.writeByte(_VL6180X_REG_SYSTEM__HISTORY_CTRL, 0x01) #Enable history
        self.writeByte(_VL6180X_REG_SYSRANGE_START, 0x03)  # Sysrange start continuous
        
    def readContRange(self):
    	return self.readByte(_VL6180X_REG_HIST_RESULT)
